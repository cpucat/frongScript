...
log = logging.getLogger("frongpy.VM")
...
class Machine:
    """
    Virtual machine for the frong programming language.
    Full specification can be found here: https://esolangs.org/wiki/frong
    """
    ...
    def dispatch(self, op: Token):
        ...
        See: https://esolangs.org/wiki/frong#Instructions
        """
        log.debug(f"Evaluating {str(op)}")

        if op is OPCODE.frong:
            log.debug("Pushing 'frong' to stack")
            self.push("frong")
            ...
            # We return an empty string to ensure the 99 frongs example doesn't crash,
            # as it relies in UB.
            try:
